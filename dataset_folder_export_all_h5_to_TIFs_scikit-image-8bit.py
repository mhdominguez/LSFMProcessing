...
# this script converts an entire 8-bit H5 dataset to TIF, excluding dataset.xml
# usage: python3 dataset_folder_export_all_h5_to_TIFs_scikit-image-8bit.py
	...
	for filename in files:
		if re.search("\d\.h5",filename): #( filename.endswith("00.h5") or filename.endswith("01.h5") ):
			...
			for this_name in list(subgroup_names):
				...
				image = np.array(f["/"+last_group_name+"/"+this_name+"/0/cells"]).astype("uint8")
